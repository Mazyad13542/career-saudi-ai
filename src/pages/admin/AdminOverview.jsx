import { Users, Briefcase, FileText, CreditCard, TrendingUp, Loader2 } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="space-y-3">
            {[
              { label: 'وظائف نشطة',      value: stats?.activeJobs ?? 0,      icon: '✅' },
              { label: 'اشتراكات مكتملة', value: stats?.totalPayments ?? 0,   icon: '💳' },
              { label: 'مستخدمو HR',       value: hrUsers,                     icon: '👔' },
              { label: 'إجمالي الوظائف',  value: stats?.totalJobs ?? 0,       icon: '📋' },
            ].map((item) => (
              <div key={item.label} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl flex-row-reverse">
                <span className="text-lg">{item.icon}</span>
                <span className="flex-1 text-sm font-bold text-gray-700 text-right">{item.label}</span>
                <span className="font-black text-[#006C35]">{item.value}</span>
              </div>
            ))}
          </div>
        </div>
